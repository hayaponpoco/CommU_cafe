...
### divide by staff
```{r}
# data of robot condition
dr <- subset(d, staff=="r")

# data of human condition
dh <- subset(d, staff=="h")
```
...
### t-test
```{r}
t.test(dr$amount, dh$amount, alternative = "two.sided", var.equal = F, paired = F)
```


### 1-factor
```{r}
res.aov <- aov(amount ~ staff, data = d)
summary(res.aov)
```
...
```{r}
st.data <- stack(dq)
histogram(~ values | ind, data = st.data, nint = 7, layout = c(5, 5))
```


### polychoric
```{r}
res.fa <- fa(dq, 2, fm="ml", rotate = "promax")
```
